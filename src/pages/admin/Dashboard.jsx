    ...
    <>
      {(theatre || city) && (
        <div className="mb-2 text-lg font-semibold text-primary">
          {theatre && <>Theatre: <span className="text-black">{theatre}</span></>}
          {city && <><br/>City: <span className="text-black">{city}</span></>}
        </div>
      )}
            ...
            <div className="relative flex flex-wrap gap-6 mt-4 max-w-5xl">
                {dashboardData.activeShows.map((show) => (
                    <div key={show._id} className="w-55 rounded-lg overflow-hidden h-full pb-3 bg-primary/10 border border-primary/20 hover:-translate-y-1 transition duration-300">
                        ...
                        <p className="font-medium p-2 truncate alta-font">{show.movie.title}</p>
